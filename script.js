...
function generateMainWallet() {
  try {
    ...
    mainWalletDisplay.textContent = mainWallet.address;
    copyMainWallet.style.display = "inline-block";
    document.getElementById("depositAndTransfer").disabled = !connectedAddress;
    log(`✅ Main wallet generated: ${mainWallet.address}`, "green");
    const content = `Main Wallet: ${mainWallet.address} | ${mainWallet.privateKey}\n`;
    const blob = new Blob([content], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "main_wallet.txt";
    a.click();
...
function connectWallet() {
  ...
  window.ethereum.request({ method: "eth_requestAccounts" })
    .then(accounts => {
      ...
      document.getElementById("depositAndTransfer").disabled = !mainWallet;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  elements.copyMainWallet.addEventListener("click", () => {
    if (!mainWallet) {
      log("❌ No main wallet generated", "red");
      return;
    }
    navigator.clipboard.writeText(mainWallet.address)
      .then(() => log("✅ Main wallet address copied", "green"))
      .catch(error => log(`❌ Failed to copy address: ${error.message}`, "red"));
  });
  elements.calculateTotal.addEventListener("click", calculateTotal);
  elements.depositAndTransfer.addEventListener("click", () => {
    const chainId = document.getElementById("chainSelect").value;
    window.depositAndTransfer(provider, signer, connectedAddress, mainWallet, lastCalculatedTotal, chainId, log);
  });
